...
class MainScreenState extends State < MainScreen > {
    ...
    Future < List < Widget >> feedItems() async {
        ...
        for (String day in absents.keys.toList()) feedCards.add(new AbsenceCard(absents[day], globals.isSingle, context));
        ...
        for (Note note in notes) feedCards.add(new NoteCard(note, globals.isSingle, context));
        for (Lesson l in lessons.where((Lesson lesson) => (lesson.isMissed || lesson.isSubstitution) && lesson.date.isAfter(now))) feedCards.add(ChangedLessonCard(l, context));
        ...
        */
        ...
        if (realLessons.length > 0 && isLessonsTomorrow) feedCards.add(new TomorrowLessonCard(realLessons, context, now));
        try {
            feedCards.sort((Widget a, Widget b) {
                return b.key.toString().compareTo(a.key.toString());
            });
        ...
        if (maximumFeedLength > feedCards.length) maximumFeedLength = feedCards.length;
        return feedCards.sublist(0, maximumFeedLength);
    }
    Future < bool > _onWillPop() {
        return showDialog(context: context, builder: (BuildContext context) {
            return new AlertDialog(title: new Text(S.of(context).sure), content: new Text(S.of(context).confirm_close), actions: < Widget > [
                new FlatButton(onPressed: () => Navigator.of(context).pop(false), child: new Text(S.of(context).no), ),
    ...
    Widget build(BuildContext context) {
        return new WillPopScope(onWillPop: _onWillPop, child: Scaffold(drawer: GDrawer(), appBar: new AppBar(title: new Text(globals.isSingle ? globals.selectedAccount.user.name : S.of(context).title), actions: < Widget > [
            //todo search maybe?
        ], ), body: hasOfflineLoaded && globals.isColor != null && mainScreenCards != null ? new Container(child: Column(children: < Widget > [!hasLoaded ? Container(child: new LinearProgressIndicator(value: null, ), height: 3, ) : Container(height: 3, ),
            new Expanded(child: new RefreshIndicator(child: new ListView(children: mainScreenCards, ), onRefresh: () {
    ...
    Future < Null > _onRefresh({
        ...
        setState(() {
            if (offline) hasOfflineLoaded = false;
            else hasLoaded = false;
        });
        if (globals.isSingle) {
            try {
        ...
        if (offline) {
            ...
            try {
                lessons = await getLessons(startDate, startDate.add(Duration(days: 6)), globals.selectedUser, showErrors);
            } catch (exception) {
                print(exception);
            }
        }
