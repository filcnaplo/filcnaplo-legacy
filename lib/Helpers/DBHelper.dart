...
class DBHelper {
    ...
    Future < void > addStudentJson(Map json, User user) async {
        ...
        if (studentJson == null) await store.record(user.id.toString() + '_student_json').add(db, json);
        else await store.record(user.id.toString() + '_student_json').update(db, json);
    }
    Future < Map > getStudentJson(User user) async {
        return await store.record(user.id.toString() + '_student_json').get(db) as Map;
    }
    Future < void > addMessagesJson(List json, User user) async {
        ...
        if (studentJson == null) await store.record(user.id.toString() + '_messages_json').add(db, json);
        else await store.record(user.id.toString() + '_messages_json').update(db, json);
    }
    Future < List > getMessagesJson(User user) async {
        return await store.record(user.id.toString() + '_messages_json').get(db) as List;
    }
    Future < void > addTestsJson(List json, User user) async {
        ...
        if (testsJson == null) await store.record(user.id.toString() + '_tests_json').add(db, json);
        else await store.record(user.id.toString() + '_tests_json').update(db, json);
    }
    Future < List > getTestsJson(User user) async {
        return await store.record(user.id.toString() + '_tests_json').get(db) as List;
    }
    Future < void > addMessageByIdJson(int id, Map < String, dynamic > json, User user) async {
        ...
        if (studentJson == null) await store.record(name).add(db, json);
        else await store.record(name).update(db, json);
    }
    ...
    Future < void > saveSettingsMap(Map json) async {
        ...
        if (settingsMap == null) await store.record('settings').add(db, json);
        else await store.record('settings').update(db, json);
    }
    Future < Map > getSettingsMap() async {
        return await store.record('settings').get(db) as Map;
    }
    Future < void > saveTimetableMap(String time, User user, List < dynamic > json) async {
        ...
        if (timetableMap == null) await store.record('timetable_' + time + user.id.toString()).add(db, json);
        else await store.record('timetable_' + time + user.id.toString()).update(db, json);
    }
    Future < List < dynamic >> getTimetableMap(String time, User user) async {
        return await store.record('timetable_' + time + user.id.toString()).get(db) as List < dynamic > ;
    }
}