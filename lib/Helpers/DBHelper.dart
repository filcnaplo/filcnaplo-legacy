...
class DBHelper {
  ...
  Future<Map> getStudentJson(User user) async {
    return await store.record(user.id.toString() + '_student_json').get(db)
        as Map;
  }
  ...
  Future<void> addMessagesJson(List json, User user) async {
    ...
    if (studentJson == null)
      ...
      await store
          .record(user.id.toString() + '_messages_json')
          .update(db, json);
  }

  Future<List> getMessagesJson(User user) async {
    return await store.record(user.id.toString() + '_messages_json').get(db)
        as List;
  }
  ...
  Future<List> getTestsJson(User user) async {
    return await store.record(user.id.toString() + '_tests_json').get(db)
        as List;
  }

  Future<void> addMessageByIdJson(
      int id, Map<String, dynamic> json, User user) async {
  ...
  Future<void> saveTimetableMap(
      String time, User user, List<dynamic> json) async {
    ...
    if (timetableMap == null)
      await store
          .record('timetable_' + time + user.id.toString())
          .add(db, json);
    else
      await store
          .record('timetable_' + time + user.id.toString())
  ...
  Future<List<dynamic>> getTimetableMap(String time, User user) async {
    return await store.record('timetable_' + time + user.id.toString()).get(db)
        as List<dynamic>;
  }
}
